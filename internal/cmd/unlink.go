...
type UnlinkCmd struct {
	Skill  string   `arg:"" help:"Skill name to unlink."`
	Target []string `name:"target" help:"Target agent to unlink from: codex or amp."`
...
func (c UnlinkCmd) Run(ctx *Context) error {
	...
	next, result, err := reconciler.Unlink(lock, c.Skill, c.Target, syncer.Options{DryRun: c.DryRun, Force: c.Force})
	if err != nil {
		return err
	}
	if !c.DryRun {
